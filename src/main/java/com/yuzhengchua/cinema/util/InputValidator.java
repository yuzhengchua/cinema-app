...
/**
 * The {@code InputValidator} class provides utility methods for validating
 * various user inputs
 * related to cinema seat booking, including seat map formats, seat
 * availability, and seat selection formats.
 * This class is not intended to be instantiated, as it contains only static
 * methods.
 * <p>
 * It includes methods for validating seat map input format, checking if a
 * string is numeric,
 * ensuring valid row and seat configurations, and verifying seat selection
 * format.
 * </p>
 * 
 * @since 1.0
...
public class InputValidator {
    ...
    /**
     * Validates the format of the seat map input. The input must consist of three
     * elements:
     ...
     * 
     * @param input The input array consisting of the movie title, number of rows,
     *              and seats per row.
    ...
    public static boolean isValidSeatMapFormat(String[] input) {
        ...
        String col;
        try {
            String[] movieString = MovieSeatMapGenerator.generateMovieSeatMap(input);
            movieTitle = movieString[0];
            row = movieString[1];
            col = movieString[2];
        ...
        return movieTitle.trim().matches("[a-zA-Z0-9 ]+") && isNumeric(row)
                && isNumeric(col);
    }
     ...
     * @return {@code true} if the string is a valid numeric value; {@code false}
     *         otherwise.
     */
    ...
    /**
     * Validates whether the number of rows and seats per row are within valid
     * bounds.
     * The number of rows and seats per row must be positive and fall within a
     * predefined range
     ...
     * @return {@code true} if the row and seat values are valid; {@code false}
     *         otherwise.
     */
    ...
    /**
     * Validates the format of a chosen seat. The seat format must match a specific
     * pattern,
